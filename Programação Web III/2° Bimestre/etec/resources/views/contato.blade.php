...
<body class="bg-gray-50 text-gray-800 min-h-screen flex flex-col justify-between">
    ...
    <header class="p-8">
        <div class="container mx-auto flex justify-between items-center">
            ...
            <nav class="flex items-center space-x-1 font-medium text-sm">
                ...
                <a href="{{ route('contato.form') }}" class="bg-[#B20000] text-white px-10 py-2.5 rounded-full transition">Contato</a>
            </nav>
    ...
    <main class="container mx-auto px-6 py-12 flex-grow max-w-5xl">
        ...
        <div class="mb-12 text-center md:text-center">
            <h2 class="text-3xl font-extrabold text-gray-900">Fale <span class="text-[#B20000]">Conosco</span></h2>
            <p class="text-gray-500 mt-2">Dúvidas sobre o Vestibulinho, cursos ou eventos? Envie uma mensagem para nossa secretaria.</p>
        </div>

        <div class="max-w-2xl mx-auto bg-white rounded-3xl p-8 shadow-xs border border-gray-100 mb-16">

            <!-- Mensagem de Sucesso (Exibida se o redirecionamento do back-end contiver a sessão) -->
            @if(session('sucesso'))
                <div class="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-2xl mb-6 text-sm font-medium">
                    {{ session('sucesso') }}
                </div>
            @endif
            ...
            <form action="{{ route('contato.enviar') }}" method="POST" class="space-y-6">
                ...
                <!-- ========================================== -->
                <!-- Comentário no Algoritmo: A diretiva @csrf gera um input oculto contendo um token
                     de segurança único para o ciclo desta sessão, impedindo ataques do tipo
                     Cross-Site Request Forgery ao submeter dados via requisição POST. -->
                @csrf

                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Nome Completo</label>
                    <input type="text" name="nome" required class="w-full bg-gray-50 border border-gray-200 rounded-2xl px-4 py-3 focus:outline-none focus:border-[#B20000] focus:bg-white text-sm transition-all">
                </div>

                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">E-mail para Contato</label>
                    <input type="email" name="email" required class="w-full bg-gray-50 border border-gray-200 rounded-2xl px-4 py-3 focus:outline-none focus:border-[#B20000] focus:bg-white text-sm transition-all" placeholder="exemplo@email.com">
                </div>

                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Mensagem ou Dúvida</label>
                    <textarea name="mensagem" rows="5" required class="w-full bg-gray-50 border border-gray-200 rounded-2xl px-4 py-3 focus:outline-none focus:border-[#B20000] focus:bg-white text-sm transition-all" placeholder="Escreva aqui sua mensagem detalhadamente..."></textarea>
                </div>

                <div class="pt-2">
                    <button type="submit" class="w-full bg-[#B20000] hover:bg-red-800 text-white py-3.5 rounded-3xl transition duration-300 text-sm shadow-md shadow-red-700/10 cursor-pointer">
                        Enviar Mensagem
                    </button>
                </div>
            </form>
